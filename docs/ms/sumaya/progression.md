---
title: Kemajuan
description: 'XP, tahap, pangkat, dan jalan terpantas untuk naik di Sumaya.'
lang: ms
translated_from_hash: 0b9aefb0
translated_on: '2026-08-07'
needs_review: true
---


# Kemajuan

Sumaya mempunyai **sistem XP dua peringkat**: setiap tindakan yang anda lakukan memperoleh **XP pemain** (untuk tahap global anda) dan **XP sektor** (untuk kerjaya atau perkhidmatan tertentu yang anda lakukan). Halaman ini menerangkan bagaimana semuanya berfungsi - apa yang naik tahap pada bila masa, bagaimana pangkat sesuai, dan strategi mana yang paling berkesan.

<ProgressionTrack>
  <ProgressionLane eyebrow="Sentiasa aktif" label="Tahap pemain" value="1-300" tone="player">Tahap bersama anda meningkat merentasi setiap kerjaya, perkhidmatan, dan misi.</ProgressionLane>
  <ProgressionLane eyebrow="Kerjaya semasa" label="Tahap sektor" value="1-100" tone="sector">Setiap sektor mempunyai XP, pangkat, pakaian, dan pembukaan kunci kenderaannya sendiri.</ProgressionLane>
</ProgressionTrack>

<WaypointSteps>
  <WaypointStep number="1" title="Permainan awal, tahap 1-10">Ikuti misi alu-aluan, cuba peralatan percuma, dan pelajari satu cara yang boleh dipercayai untuk memperoleh XP.</WaypointStep>
  <WaypointStep number="2" title="Permainan pertengahan, tahap 10-50">Bina satu sektor utama sambil menggunakan misi dan kerjaya lain untuk terus menggerakkan XP pemain.</WaypointStep>
  <WaypointStep number="3" title="Permainan akhir, tahap 50+">Sasarkan pangkat dan pembukaan kunci kenderaan mahal yang paling penting untuk peranan pilihan anda.</WaypointStep>
</WaypointSteps>

## Dua peringkat

### Tahap pemain

- Tahap akaun global, dikongsi merentasi semua perkara yang anda lakukan.
- Berjulat dari **1 hingga 300**.
- Setiap tindakan sektor menambah XP pemain anda secara selari dengan XP sektor anda.
- Tahap pemain yang lebih tinggi membuka kunci beberapa kenderaan (kebanyakannya Polis) dan pencapaian rantaian quest.
- Untuk naik tahap dari 1 hingga 300 memerlukan kira-kira **4.5 juta jumlah XP** - ia adalah grind yang panjang, kebanyakan pemain akan berhenti jauh sebelum 300.

### Tahap sektor

- Tahap setiap kerjaya, bebas untuk setiap sektor yang anda kerjakan.
- Setiap sektor kini berjulat dari **1 hingga 100** (Bomba, Kesihatan, Polis, Fishing, Service, dan Agriculture/palm oil semuanya berkongsi tangga panjang yang sama).
- Tahap sektor anda menentukan **pangkat** (gelaran) anda dalam sektor itu.
- Tahap sektor yang lebih tinggi membuka kunci kenderaan khusus pasukan (cth. Sanica Tanker pada sektor Bomba 65).

### Bagaimana mereka naik tahap bersama

Hampir setiap tindakan dalam permainan memberi **kedua-dua** XP pemain dan XP sektor, dengan jumlah yang sama untuk setiap satu. Sebagai contoh, memadamkan satu blok api memberi 5 XP pemain DAN 5 XP sektor Bomba.

Jadi semasa anda grind, kedua-dua akaun global anda dan sektor tertentu meningkat secara selari.

## XP setiap tindakan (rujukan pantas)

Berikut adalah berapa banyak XP yang diberikan oleh tindakan yang berbeza. Kedua-dua XP pemain dan XP sektor, melainkan dinyatakan.

### Kerjaya awam

| Tindakan | XP (skala dengan tahap) |
|--------|------------------------|
| Memancing dengan joran (per ikan dijual, 5 spesies) | 15 XP kepada pemain sahaja - TIDAK memajukan sektor fishing |
| Pukat tunda (per ikan ditarik) | 2 XP kepada pemain dan sektor fishing |
| Selesaikan pesanan mamak / makanan segera | Tetap **6 XP** setiap pelanggan + **bonus 10 XP akhir syif** jika minimum disajikan. Tiada skala tahap. |
| Tuaian kelapa sawit | ~5 XP pada tahap 1, meningkat dengan tahap sektor |
| Bonus akhir syif kelapa sawit (XP pemain sahaja) | 50 XP |

### Perkhidmatan kecemasan

| Tindakan | XP |
|--------|-----|
| Bomba: padamkan 1 blok api | 5 XP |
| Bomba: EMS lain berdekatan | 10 XP (kepada pembantu bukan Bomba) |
| Kesihatan: diagnosis bantuan kecemasan | 15 XP |
| Kesihatan: rawatan pembalut | 30 XP |
| Kesihatan: rawatan splint | 40 XP |
| Kesihatan: diagnosis X-Ray / MRI | 20 XP |
| Polis: tangkap suspek 1 bintang | 50 XP |
| Polis: tangkap suspek 2 bintang | 100 XP |
| Polis: tangkap suspek 3 bintang | 175 XP |
| Polis: tangkap suspek 4 bintang | 275 XP |
| Polis: tangkap suspek 5 bintang | 400 XP |
| Polis: pegawai berdekatan | 50% daripada ganjaran pegawai yang menangkap |
| Polis: tangkapan radar | 30 XP |

### Quest

Rantaian penemuan 12-quest membayar **3,750 XP** merentasi ganjaran setiap quest, **tambah bonus penamat 2,000 XP sekali sahaja** pada quest terakhir. Diselesaikan sepenuhnya = **5,750 XP**. Pembayaran SR juga sama: 2,900 biasa + 1,500 penamat = 4,400 Ringgit. Lihat [Quests](/ms/sumaya/quests).

## Boost XP +15% Kesihatan

Sistem kecederaan **diaktifkan secara automatik bila-bila masa mana-mana pemain Kesihatan berada di server**. Semasa ia aktif, setiap pemain mendapat **pengganda +15% pada XP pemain**. Boost dikenakan kepada semua sumber - jenayah, kerja, kebakaran, apa-apa sahaja yang memberi XP pemain.

XP sektor **tidak** diboost - hanya XP pemain. Jadi boost membantu grind akaun global anda, bukan kerjaya tertentu anda.

Ini adalah insentif kuat untuk mempunyai pemain perubatan aktif di server anda. Sesi grind yang panjang jauh lebih cekap apabila ia diaktifkan.

## Pakaian dan kit kerjaya

Setiap pasukan perkhidmatan kecemasan (POLIS, BOMBA, Kesihatan) mempunyai aliran aplikasi **Careers** di mana anda melengkapkan pakaian dan barangan kit yang dibuka kunci. Item-item ini disekat oleh **tahap sektor** anda untuk pasukan itu:

- **Pakaian** menukar penampilan uniform watak anda (cth. Nurse scrubs, Specialist firesuit, Ambulance Operator vest).
- **Item kit** memasangkan peralatan (tali pinggang, topi keledar, topeng, baju hujan).

Item terkunci menunjukkan "Locked (Lv. X)" dalam aplikasi Careers sehingga tahap sektor anda mencapai ambang. Contoh sekatan:

- **POLIS**: Duty Belt + Beret pada L1, Reflective Vest pada L10, Reflective Coat pada L20.
- **BOMBA**: Respirator + SCBA pada L15, EMRS pada L20, pelbagai kit pada L30-40, Senior Firesuit pada L50, Command Helmet pada L60, pakaian Calendar pada L67.
- **Kesihatan**: Nurse + Medical Mask pada L1, Ambulance Operator + Medikal Cap pada L15, N95 pada L20, Medikal Reflective Vest pada L25, Scrubs + Face Shield pada L30.

## Pangkat

Setiap sektor perkhidmatan awam mempunyai **pangkat** - gelaran yang dikemas kini secara automatik berdasarkan tahap sektor anda. Mereka adalah kosmetik; mereka tidak menyekat permainan (hanya tahap anda yang menyekat).

### Pangkat Bomba (13 jumlah)

| Tahap | Pangkat |
|-------|------|
| 1 | Pegawai Bomba (PB) |
| 6 | Pegawai Bomba Kanan (PBK) |
| 11 | Pegawai Bomba Tinggi (PBT) |
| 16 | Penolong Penguasa Bomba (PPgB) |
| 21 | Penolong Kanan Penguasa Bomba (PKPgB) |
| 26 | Timbalan Penguasa Bomba (TPgB) |
| 31 | Penolong Penguasa Bomba (Tahap Stesen) |
| 36 | Penguasa Bomba (PgB) |
| 41 | Penguasa Kanan Bomba (PgKB) |
| 49 | Penolong Pesuruhjaya Bomba (PPjB) |
| 57 | Penolong Kanan Pesuruhjaya Bomba (PKPjB) |
| 65 | Timbalan Pesuruhjaya Bomba (TPjB) |
| 76 | Pesuruhjaya Bomba (PjB) |

### Pangkat Kesihatan (13 jumlah)

| Tahap | Pangkat |
|-------|------|
| 1 | Pelatih Perubatan |
| 6 | Pegawai Rumah |
| 11 | Jururawat |
| 16 | Pegawai Perubatan |
| 21 | Penolong Pegawai Perubatan |
| 26 | Penolong Kanan Pegawai Perubatan |
| 31 | Timbalan Ketua Penolong Pegawai Perubatan |
| 36 | Ketua Penolong Pegawai Perubatan |
| 41 | Timbalan Pengarah Kesihatan |
| 49 | Pengarah Kesihatan |
| 57 | Pengarah Kanan Kesihatan |
| 66 | Timbalan Ketua Pengarah Kesihatan |
| 81 | Ketua Pengarah Kesihatan |

### Pangkat Polis (16 jumlah)

| Tahap | Pangkat |
|-------|------|
| 1 | Konstabel Polis |
| 6 | Lans Koperal (L/Cpl) |
| 11 | Koperal (Cpl) |
| 16 | Sarjan (Sgt) |
| 21 | Sarjan Mejar (SM) |
| 26 | Sub-Inspektor (SI) |
| 31 | Inspektor Percubaan (P/Insp) |
| 36 | Inspektor (Insp) |
| 41 | Ketua Inspektor (C/Insp) |
| 46 | Penolong Penguasa Polis (ASP) |
| 51 | Timbalan Penguasa Polis (DSP) |
| 56 | Penguasa Polis (SUPT) |
| 61 | Penolong Pesuruhjaya Polis (ACP) |
| 69 | Penolong Kanan Pesuruhjaya Polis (SAC) |
| 77 | Timbalan Pesuruhjaya Polis (DCP) |
| 89 | Pesuruhjaya Polis (CP) |

Untuk ketiga-tiga perkhidmatan awam, had pangkat adalah di bawah had tahap. Anda terus memperoleh XP melepasi pangkat akhir, tetapi gelaran anda tidak akan berubah.

## Kandungan disekat-tahap

Beberapa perkara dibuka kunci apabila anda mencapai tahap atau tahap sektor tertentu. Kebanyakannya adalah kenderaan.

### Sekatan tahap pemain

| Tahap | Buka kunci |
|-------|--------|
| 1 | Permulaan permainan - kerja awam, kenderaan lalai, Tier 1 Trawler |
| 2 | Selesaikan quest `q_levelup` (ganjaran 300 XP, 200 SR) |
| 5 | Selesaikan quest `q_rising_star` (500 XP, 400 SR) |
| 10 | Selesaikan quest `q_veteran` (800 XP, 700 SR) |
| 15 | Protan Waja MPV Polis (6,500 Ringgit) - Polis sahaja |

### Sekatan tahap sektor (Bomba)

| Tahap sektor | Buka kunci |
|--------------|--------|
| 1 | Merze Atego LFRT (percuma) |
| 20 | Sanica 94D LFRT (9,000 Ringgit) |
| 40 | Tayoti Hiace Bomba (12,000 Ringgit), Sanica P310 FRT (13,000 Ringgit) |
| 65 | Sanica P360 Tanker (20,000 Ringgit) |

### Sekatan tahap sektor (Polis)

| Tahap sektor | Buka kunci |
|--------------|--------|
| 1 | Protan Wira MPV Polis (percuma) |
| 35 | Handai City MPV Polis (12,000 Ringgit) |
| 55 | Handai Civic MPV Polis (15,000 Ringgit) |

### Sekatan tahap sektor (Kesihatan)

| Tahap sektor | Buka kunci |
|--------------|--------|
| 1 | Tayoti Hiace KKN Ambulance (percuma) |
| 20 | Tayoti Hilux KKN Ambulance (8,000 Ringgit) |
| 45 | Handai City KKN Ambulance (12,000 Ringgit) |

### Sekatan sektor awam

| Acara | Syarat |
|-------|-----------|
| Buka kunci Sektor Service (Mamak / Makanan Segera) | Capai tahap 50 dalam Fishing atau Agro |
| Akses Tier 2 / Tier 3 Trawlers | Miliki Ringgit yang cukup untuk membelinya |

## Strategi naik tahap

Mahu naik tahap dengan cepat? Berikut adalah taktik kasar:

### Permainan awal (tahap pemain 1-10)

- Buat rantaian quest. Ia diisi awal dengan pencapaian tahap 2, 5, dan 10, membayar 300+500+800 = 1,600 XP dalam kepingan.
- Pilih sektor awam untuk clock in semasa anda menyelesaikan langkah quest yang lain.
- Jangan risau tentang tahap sektor lagi - fokus pada XP pemain.

### Permainan pertengahan (tahap pemain 10-50)

- Pilih perkhidmatan kecemasan jika anda belum. Boost +15% Kesihatan membantu semua orang.
- Bomba bagus untuk grind kumpulan (kebakaran lebih besar, lebih banyak pemain = bonus pencapaian lebih cepat).
- Polis pada waktu sepi adalah perlahan - anda perlukan penjenayah dalam talian.

### Permainan lewat (tahap pemain 50+)

- Syif kelapa sawit untuk bonus 50 XP akhir syif.
- Tangkapan 5 bintang jika anda boleh mendapatkannya (400 XP setiap satu).
- Bonus kebakaran Bomba setiap 15 blok = 150 SR.

### Strategi pelbagai sektor

- Semua yang anda peroleh dalam sektor menaikkan akaun pemain anda secara selari.
- Setiap sektor kini menetap pada tahap 100 - tidak lagi ada sektor "tangga terpendek". Pilih kadar mana yang sesuai dengan apa yang anda nikmati.
- Fishing adalah paling pasif - naikkan tahapnya semasa anda melakukan perkara lain.

## Pemindahan XP berlebihan

Jika anda mempunyai cukup XP untuk naik tahap beberapa kali dalam satu tindakan (cth. tangkapan besar 400 XP pada tahap rendah), lebihan dipindahkan dengan betul - anda akan maju melalui beberapa tahap sekaligus tanpa kehilangan XP.

## Petua

- **Jejak boost +15%.** Sesi panjang + Kesihatan aktif = jauh lebih cekap.
- **Jangan lupa XP pemain berasingan daripada XP sektor.** Jika anda menukar kerjaya, kedua-duanya masih membesar.
- **Pangkat adalah flex, bukan permainan.** Jangan rasa tertekan untuk maksimumkan pangkat yang anda tidak nikmati.
- **Sekatan tahap adalah lebih sedikit daripada yang dilihat.** Kebanyakan kandungan tersedia dari tahap 1. Sekatan besar adalah untuk kenderaan pasukan tahap tinggi.

## Apa seterusnya?

- [Economy](/ms/sumaya/economy) untuk bagaimana Ringgit berfungsi secara selari.
- [Quests](/ms/sumaya/quests) untuk rantaian 12-quest dan pencapaian.
- [Career overview](/ms/sumaya/jobs) atau [Emergency services overview](/ms/sumaya/emergency-services) untuk petua grind khusus sektor.
