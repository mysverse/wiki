...
| 行业 | 工作内容 | 最高等级 | 启动条件 |
|--------|-------------|-----------|-------------------|
| Fishing | 在码头使用钓竿或在船上拖网 | 100 | 无 |
| Service (Mamak / Fast Food) | 上菜、接订单 | 100 | 在 fishing 或 agriculture 达到行业等级 50 |
...
1. 打开您的**手机**并点击 **Careers** 应用程序。
2. 选择一个行业。如果它被锁定,应用程序会告知您原因。
3. 走到世界中对应的**职业站点**(例如捕鱼码头、mamak 摊位、棕榈种植园)。Careers 应用程序与任务指引光柱会为您指引方向。
4. 按下 **interact** 提示以打卡上工。
...
打开 Careers 应用程序并点击当前工作以停止。某些行业(例如 palm oil)只在**您打卡下工时**才发放您的收入,因此请勿忘记结束您的轮班。
...
1. **玩家 XP** - 提升您的全局账号等级。玩家最高等级为 300。
2. **行业 XP** - 提升您当前从事的特定行业等级。

动作的报酬取决于行业。Trawl fishing 每次捕获给予 2 XP(rod fishing 每个鱼种支付 15 XP - 仅玩家 XP,无行业 XP)。Service 行业固定每位顾客支付 6 XP(无等级缩放)外加 10 XP 的轮班结束奖励。Palm oil 会储存您的收入,并在轮班结束时支付 50 XP 奖励。

请参阅各行业页面以获取数据,并参阅 [Progression](/sumaya/progression) 了解 XP 如何转换为等级。
